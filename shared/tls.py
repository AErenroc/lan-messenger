...
def server_ssl_context(
    ...
    """
    Server SSLContext mith mTLS
    - Presents its own cert (signed by CA)
    - Verifies the client's cert against the CA
    """
    # Ensure CA exists, then the server cert
    generate_ca(ca_cert_path, ca_key_path)
    if not cert_path.exists() or not key_path.exists():
        log.info("No server cert found - - generating server cert for [%s] ...", host_addr)
        generate_server_cert(host_addr, cert_path, key_path, ca_cert_path, ca_key_path)

    ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
    ctx.minimum_version = _MIN_TLS
    ctx.set_ciphers(_CIPHERS)

    # Servers own identity
    ctx.load_cert_chain(certfile=str(cert_path), keyfile=str(key_path))

    # Trust only certs signed by our CA - - enforce mTLS
    ctx.load_verify_locations(cafile=str(ca_cert_path))
    ctx.verify_mode = ssl.CERT_REQUIRED   # reject clients with no/wrong cert
...
def client_ssl_context(
    ...
    """
    Client SSLContext with mTLS
    - Presents its own cert (signed by CA)
    - Verifies the server's cert against the CA
    """
    for p, label in [(cert_path, "client cert"), (key_path, "client key"), (ca_cert_path, "CA cert")]:
        if not p.exists():
            print(f"[!] {label} not found at {p}. Exiting.")
            sys.exit(1)
    ...
    # Client's own identity (presented to server)
    ctx.load_cert_chain(certfile=str(cert_path), keyfile=str(key_path))

    # Verify the server against the CA
    ctx.load_verify_locations(cafile=str(ca_cert_path))
    ctx.verify_mode    = ssl.CERT_REQUIRED
    ctx.check_hostname = True  # LAN IPs - hostname check uses SAN
